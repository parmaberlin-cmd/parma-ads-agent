...
const allowedRoots = {
  ...
  intelligence: ['success', 'source', 'mode', 'reader_version', 'campaign_id', 'period_days', 'date_range',
    'requested_period_days', 'read_mode', 'exact_date_range', 'configured_state', 'observed_performance',
    'inferred_diagnosis', 'overview', 'ad_groups', 'search_terms', 'keywords', 'devices', 'hours', 'geography',
...
function createLocalReader(config, client = axios) {
  return async ({ method, path: endpoint, query }) => {
    ...
    if (method !== 'GET' || (!intelligence && !Object.hasOwn(allowedRoots, endpoint)) ||
        !query || typeof query !== 'object' || Array.isArray(query)) throw new Error('read_not_allowed');
    const queryKeys = Object.keys(query);
    if (intelligence) {
      if (queryKeys.some(key => !['days', 'read_mode'].includes(key)) ||
          !Number.isInteger(query.days) || query.days < 0 || query.days > 90 ||
          (query.read_mode !== undefined && !['historical', 'today_intraday'].includes(query.read_mode))) {
        throw new Error('read_not_allowed');
      }
