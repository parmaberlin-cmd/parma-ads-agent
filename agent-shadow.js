...
function deriveLiveFunnel(input={}){
  ...
  const events=input.ga4_funnel?.reservation_funnel?.google_cpc;if(!events)return{};
  return {landingAvailable:true,adClicks:Number(input.google?.totals?.clicks||input.current?.clicks||0),landingViews:ga4EventCount(events,"page_view"),reservationStarts:ga4EventCount(events,"booking_started"),bookings:ga4EventCount(events,"booking_completed"),bookingStartedTracked:Object.prototype.hasOwnProperty.call(events,"booking_started")};
}
...
function buildShadowAgentReport(input={}){
  ...
  const legacyDecisionSupport=buildDecisionSupportReport({meta,google,conversions});const anomalies=detectAnomalies({current:input.current||{},baseline:input.baseline||{},access:input.access||{}});
  const liveSearchTerms=input.search_terms||google.search_terms||input.live_sources?.google?.search_terms||[];const keywordInventory=google.keywords||input.live_sources?.google?.keywords||[];const searchRecommendations=analyzeSearchTerms(liveSearchTerms,{knownKeywords:keywordInventory});
  const rankedCreatives=rankCreatives(input.creatives||[]),creativeTests=proposeCreativeTests(rankedCreatives);const funnel=assessFunnel({...deriveLiveFunnel(input),conversionIntegrity:conversionIntegrity.status});
  ...
  const recommendations=[...(legacyDecisionSupport.recommendations||[]),...searchRecommendations,...creativeTests.map(item=>({...item,code:"CREATIVE_TEST_PROPOSAL",priority:"medium",score:50,reason:item.reason}))];const dailyManager=buildDailyManager({recommendations,anomalies,funnel,budget});
  const channelRoles=allocateChannelRoles(input.channel_signals||{}),businessValue=estimateBusinessValue(input.business_value||{}),schedule=scheduleChecks({now:input.now?new Date(input.now):new Date(),lastRuns:input.last_runs||{}});const actions=dailyManager.primary_priorities.map(priority=>({code:priority.code,safety:classifyAction({type:"analyze"})}));
  const journal=createDecisionJournalEntry({channel:"cross_channel",evidenceWindow:input.evidence_window||null,evidence:{conversion_integrity:conversionIntegrity,anomaly_count:anomalies.length,priority_count:dailyManager.primary_priorities.length,search_terms_analyzed:liveSearchTerms.length,keyword_inventory_size:keywordInventory.length,funnel_source:input.funnel?"explicit":input.ga4_funnel?.reservation_funnel?.google_cpc?"ga4_google_cpc":"unavailable",booking_started_tracked:funnel.tracking?.booking_started??null},dataQuality:conversionIntegrity.status,diagnosis:dailyManager.primary_priorities[0]?.reason||"No material priority detected.",confidence:conversionIntegrity.confidence,expectedEffect:"Shadow-mode diagnosis only; no external state mutation.",proposedAction:"review_shadow_report",requiresAuthorization:false});
