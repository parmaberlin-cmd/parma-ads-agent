...
function proposeChanges({readEvidence,context={}}){
  ...
  const proposedTarget=round2(Math.min(maxTarget,currentBudget+2.5));
  if(proposedTarget>currentBudget){
    candidates.push({action_type:'budget_adjustment',policy_name:'increase_budget',external_write:true,target:`campaign:${campaignId}:daily_budget`,current_value:{daily_budget_eur:round2(currentBudget)},proposed_value:{daily_budget_eur:proposedTarget},reason:'Create additional delivery headroom while remaining inside the authorized aggregate configured-budget cage.',evidence_refs:[ref('overview.daily_budget_eur'),ref('account_budget_context.enabled_budget_total_eur')],expected_effect:'Potentially increase eligible auction participation; actual daily cost remains governed by Google Ads delivery rules and is not a hard daily cost cap.',risk_level:'HIGH',rollback_possible:true,confidence:0.72});
  }
