...
function startReadonlyShadowScheduler({ env = process.env, client = axios } = {}) {
  ...
  if (!apiKey) {
    console.warn(JSON.stringify({ event: 'shadow_scheduler', enabled: false, reason: 'api_key_missing', writes_allowed: false }));
    return null;
  }
  ...
  async function tick() {
    try {
      const response = await client.post(url, null, { headers: { 'x-api-key': apiKey }, timeout: 30000, validateStatus: () => true });
      const ok = response.status === 202;
      console.log(JSON.stringify({ event: 'shadow_scheduler_tick', success: ok, status_code: response.status, refresh_status: response.data?.status || null, interval_minutes: intervalMinutes, writes_allowed: false }));
    } catch {
      console.error(JSON.stringify({ event: 'shadow_scheduler_tick', success: false, error: 'refresh_request_failed', interval_minutes: intervalMinutes, writes_allowed: false }));
    }
  }
  const timer = setInterval(() => { tick().catch(() => {}); }, intervalMs);
  if (typeof timer.unref === 'function') timer.unref();
  console.log(JSON.stringify({ event: 'shadow_scheduler', enabled: true, interval_minutes: intervalMinutes, writes_allowed: false }));
