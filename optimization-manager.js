...
function assessFunnel({landingAvailable=null,adClicks=null,landingViews=null,reservationStarts=null,bookings=null,conversionIntegrity="unknown",bookingStartedTracked=true}={}){
  ...
  if(bookingStartedTracked===false){if(views>=10)issues.push({code:"BOOKING_STARTED_TRACKING_MISSING",severity:"medium"});}
  else {if(views>=10&&starts/views<0.1)issues.push({code:"LANDING_TO_RESERVATION_LEAKAGE",severity:"medium"});if(starts>=5&&done/starts<0.3)issues.push({code:"RESERVATION_COMPLETION_LEAKAGE",severity:"high"});}
...
function buildDailyManager({recommendations=[], anomalies=[], funnel=null, budget=[]}={}){
  ...
  const seen=new Set();const deduped=items.sort((a,b)=>b.score-a.score||a.code.localeCompare(b.code)).filter(i=>{const key=`${i.source}:${i.code}`;if(seen.has(key))return false;seen.add(key);return true;});
