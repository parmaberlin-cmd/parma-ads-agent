...
function safeConversionConfidence(input = {}) {
  ...
  if (meta === null || ga4 === null) {
    return { confidence: 'blocked', optimization_allowed: false, reason: 'missing_or_invalid_conversion_source', writes_allowed: false };
  }
...
function safeBudgetSimulation(input = {}) {
  ...
  if (budget === null || conversionRate === null || cpc === null || budget <= 0 || cpc <= 0 || conversionRate > 1) {
    return { valid: false, reason: 'invalid_or_missing_simulation_input', scenarios: [], execution_allowed: false, writes_allowed: false };
  }
  ...
  if (Array.isArray(input.changes) && changes.length !== input.changes.length) {
    return { valid: false, reason: 'invalid_budget_change', scenarios: [], execution_allowed: false, writes_allowed: false };
  }
  const scenarios = intelligence.budgetSimulator({ currentBudget: budget, conversionRate, cpc, ...(changes ? { changes } : {}) });
  return { valid: true, reason: 'simulation_only', scenarios: scenarios.map((scenario) => ({ ...scenario, execution_allowed: false })), execution_allowed: false, writes_allowed: false };
}
...
function safeOutcomeLearning(input = {}) {
  ...
  if (before === null || after === null || !input.metric) {
    return { evaluable: false, metric: input.metric || null, before, after, delta: null, improved: null, reason: 'missing_or_invalid_outcome_evidence', writes_allowed: false };
  }
...
function assertNonExecutable(value) {
  ...
  while (stack.length) {
    ...
    for (const [key, child] of Object.entries(current)) {
      if (['writes_allowed','execution_allowed','activation_allowed','spend_allowed'].includes(key) && child === true) throw new Error(`intelligence safety contract violated: ${key}`);
...
module.exports = { finite, nonNegative, safeConversionConfidence, safeBudgetSimulation, safeOutcomeLearning, safeBudgetGuardrail, assertNonExecutable };