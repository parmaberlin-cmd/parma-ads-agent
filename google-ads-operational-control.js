...
// derived from a campaign id: campaign and customer identifiers are different
// namespaces and silently reusing one for the other produces a foreign or
// nonexistent resource at the provider boundary.
...
const campaignResource = z.string().regex(/^customers\/\d{1,20}\/campaigns\/\d{1,20}$/);
...
const actionSchema = z.discriminatedUnion('type', [
  z.object({ type: z.enum(['negative_add']), campaign_id: id, campaign_resource_name: campaignResource, text, match_type: z.enum(['EXACT', 'PHRASE']) }).strict(),
  ...
  z.object({ type: z.enum(['schedule_create']), campaign_id: id, campaign_resource_name: campaignResource, day_of_week: day, start_hour: z.number().int().min(0).max(23), start_minute: minute, end_hour: z.number().int().min(1).max(24), end_minute: minute }).strict(),
  ...
  z.object({ type: z.enum(['ad_group_create']), campaign_id: id, campaign_resource_name: campaignResource, name: z.string().trim().min(1).max(255), cpc_bid_micros: z.number().int().nonnegative().safe().optional(), status: z.literal('PAUSED') }).strict(),
  ...
  z.object({ type: z.enum(['geo_add']), campaign_id: id, campaign_resource_name: campaignResource, geo_target_constant: z.string().regex(/^geoTargetConstants\/\d+$/), negative: z.boolean().default(false) }).strict(),
  z.object({ type: z.enum(['geo_remove']), campaign_id: id, resource_name: resource }).strict(),
  z.object({ type: z.enum(['language_add']), campaign_id: id, campaign_resource_name: campaignResource, language_constant: z.string().regex(/^languageConstants\/\d+$/) }).strict(),
...
function parseAction(input) {
  ...
  assertCampaignBinding(action);
  return action;
}

// Fail-closed check: when an action targets a campaign it must name the campaign
// resource explicitly and the embedded campaign id must be the campaign the
// action claims to modify. A customer/campaign namespace mix-up (or any other
// mismatch) aborts compilation before any provider call.
function assertCampaignBinding(action) {
  if (typeof action.campaign_resource_name !== 'string') return action;
  const target = action.campaign_resource_name.match(/^customers\/(\d{1,20})\/campaigns\/(\d{1,20})$/);
  if (!target) throw new Error('invalid_campaign_resource_name');
  if (target[2] !== action.campaign_id) throw new Error('campaign_resource_campaign_mismatch');
  return action;
}
...
function compileOperation(input) {
  ...
  if (action.type === 'negative_add') return { entity: 'campaign_criterion', operation: 'create', resource: { campaign: action.campaign_resource_name, negative: true, keyword: { text: action.text, match_type: action.match_type } } };
  ...
  if (action.type === 'schedule_create') return { entity: 'campaign_criterion', operation: 'create', resource: { campaign: action.campaign_resource_name, ad_schedule: { day_of_week: action.day_of_week, start_hour: action.start_hour, start_minute: action.start_minute, end_hour: action.end_hour, end_minute: action.end_minute } } };
