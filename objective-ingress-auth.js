...
async function verify(req){const ticket=await verifier.verifySignedJwtWithCertsAsync(token(req),await certs(),AUDIENCE,[ISSUER],900);const p=ticket.getPayload();if(p.repository!==REPOSITORY||p.ref!=='refs/heads/main'||p.workflow_ref!==WORKFLOW||p.event_name!=='push')throw new Error('oidc_claims_invalid');if(!allowRate(p.repository))throw new Error('rate_limited');return p;}
