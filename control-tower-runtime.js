...
function markRetry(file,id,reason){return store.update(file,s=>{const tasks=s.tasks.map(t=>taskKey(t)===id?{...t,attempts:Number(t.attempts||0)+1,error:String(reason||'validation_failed').slice(0,120)}:t);const current=tasks.find(t=>taskKey(t)===id);const terminal=current.attempts>=Number(current.max_attempts||3);return store.appendEvent({...s,tasks,status:terminal?'BLOCKED_EXTERNAL':'FIX',current_task_id:id,blockers:terminal?[...s.blockers,{task_id:id,type:'retry_exhausted'}]:s.blockers},{type:terminal?'stop':'retry_scheduled',task_id:id,reason:current.error});});}
async function runUntilStop({file=store.statePath(),context={},executors={},validators={},maxSteps=50}={}){
  ...
  for(let step=0;step<maxSteps;step++){
    ...
    if(hasCompletedKey(state,key)){store.update(file,s=>{const tasks=s.tasks.map(t=>taskKey(t)===id?{...t,status:'done'}:t);return store.appendEvent({...s,tasks,status:'NEXT',current_task_id:null,next_task_id:null},{type:'idempotent_skip',task_id:id,idempotency_key:key});});continue;}
    ...
    try{
      ...
      if(after.state==='NEXT'){store.update(file,s=>{const tasks=s.tasks.map(t=>taskKey(t)===id?{...t,status:'done',result:result.result||null,error:null}:t);return store.appendEvent({...s,tasks,status:'NEXT',current_task_id:null},{type:'task_completed',task_id:id,idempotency_key:key});});continue;}
    ...
    }catch(error){const kind=classifyError(error);if(kind==='FIX'){const updated=markRetry(file,id,'executor_failed');if(TERMINAL.has(updated.status))return updated;continue;}return store.update(file,s=>store.appendEvent({...s,status:kind,blockers:[...s.blockers,{task_id:id,type:kind.toLowerCase()}]},{type:'stop',reason:kind.toLowerCase()}));}
  }
