...
function aggregateNamedRows(rows = [], nameFields = ['name','label']) {
  ...
  return rows.map((row) => {
    ...
    return { name:safeText(name,120) || 'unknown', gross:money(source.gross ?? source.revenueGross ?? source.totalGross ?? 0), net:money(source.net ?? source.revenueNet ?? source.totalNet ?? 0), tax:money(source.tax ?? source.vat ?? source.totalTax ?? 0), quantity:Number.isFinite(Number(source.quantity)) ? Number(source.quantity) : null };
  });
}

function containsDisallowedPii(value) {
  const text = JSON.stringify(value || {}).toLowerCase();
  return ['customer_name','customer_email','customer_phone','receipt_text','receipt_id','guest_name','first_name','last_name'].some((key) => text.includes(key));
}
...
function normalizeOrderbirdAggregate(input, { source='orderbird', timezone='Europe/Berlin' } = {}) {
  ...
  if (containsDisallowedPii(input)) throw new TypeError('pii_not_allowed');
  ...
  return {
    ...
    payment_methods: paymentRows.map((r) => ({ method:normalizePaymentMethod(r?.method ?? r?.name ?? r?.label), gross:money(r?.gross ?? r?.amount ?? r?.total ?? 0) })),
    ...
    shifts: aggregateNamedRows(input.shifts ?? [],['name','label','shift']),
    hourly: Array.isArray(input.hourly) ? input.hourly.map((r) => ({ hour:safeText(r?.hour,16), gross:money(r?.gross ?? r?.amount ?? 0), net:money(r?.net ?? 0), tax:money(r?.tax ?? r?.vat ?? 0) })) : []
  };
}
