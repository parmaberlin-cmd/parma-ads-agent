...
function budgetSimulator({ currentBudget, conversionRate, cpc, changes = [-0.2, -0.1, 0.1, 0.2] } = {}) {
  ...
  return changes.map((change) => {
    ...
    return { change, projected_budget: projectedBudget, projected_clicks: projectedClicks, projected_conversions: projectedConversions, is_simulation: true, execution_allowed: false };
  });
}
...
function evaluateWinner(rows = []) {
  ...
  const ranked = eligible.map((row) => ({ ...row, cpb: ratio(number(row.spend), number(row.bookings)) })).filter((row) => row.cpb != null).sort((a, b) => a.cpb - b.cpb);
...
function crossChannel({ google = {}, meta = {} } = {}) {
  return { google_role: "capture_existing_demand", meta_role: "generate_and_retarget_demand", warning: "do_not_compare_cpc_without_conversion_and_incrementality_context", google_cost_per_booking: ratio(number(google.spend), number(google.bookings)), meta_cost_per_booking: ratio(number(meta.spend), number(meta.bookings)) };
}
...
function autonomySchedule() {
  return { daily: ["source_health", "conversion_integrity", "anomalies", "waste", "opportunities", "priorities"], weekly: ["creative_fatigue", "audience", "placements", "cross_channel", "decision_outcomes"], conditional: ["budget_change", "experiment_winner", "write_action"], writes_allowed: false };
}
...
function fullShadowAgentV2(input = {}) {
  ...
  return { mode: "shadow_v2", writes_allowed: false, execution_allowed: false, meta_campaigns: campaigns, meta_creatives: creativeIntelligence(input.creatives || []), placements: placementIntelligence(input.placements || []), conversion_confidence: attribution, waste: wasteDetector(campaigns, input.waste_policy || {}), opportunities: opportunityDetector(campaigns, input.opportunity_policy || {}), priorities, cross_channel: crossChannel(input.channels || {}), schedule: autonomySchedule() };
}

module.exports = { classifyCampaign, campaignIntelligence, creativeIntelligence, placementIntelligence, conversionConfidence, decisionConfidence, detectMetaAnomalies, wasteDetector, opportunityDetector, budgetGuardrail, budgetSimulator, experimentPlan, evaluateWinner, fatigueDetector, audienceDiagnostics, landingAttribution, bookingValue, costPerBookingGuardrail, crossChannel, dailyManager, weeklyStrategicReport, journalEntry, outcomeLearning, agentHealth, autonomySchedule, fullShadowAgentV2 };