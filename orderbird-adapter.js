  ...
  return {
    ...
    health() {
      if (!hasTransport || transport.providerSupported !== true) {
        return { provider:'orderbird', healthy:false, usable:false, health:'unavailable', reason:'provider_supported_transport_not_verified', mutation_permission:false };
      }
      return { provider:'orderbird', healthy:true, usable:true, health:'healthy', reason:'provider_supported_read_transport_verified', mutation_permission:false };
    },

    async readAggregates(range) {
      if (!hasTransport || transport.providerSupported !== true) throw new OrderbirdProviderUnavailableError();
