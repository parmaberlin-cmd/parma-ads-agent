...
function buildConnectionsView() {
  const connections = listConnections().map((connection) => ({
    ...connection,
    health: connectionHealth(connection.id),
    human_action: humanActionNeeded(connection.id),
  }));

  return {
    success: true,
    mode: 'read_only_capability_registry',
...
function buildConnectionsHealth() {
  ...
  return {
    ...
    summary: {
      ...
      usable: connections.filter((x) => x.usable).length,
      blocked: connections.filter((x) => !x.usable).length,
    },
    connections,
  };
}
...
function resolveCapability({ connectionId, capability, mutation = false } = {}) {
  if (!connectionId || !capability) {
    return {
      success: false,
      allowed: false,
      reason: 'connection_id_and_capability_required',
      mutation_permission: false,
    };
  }
  ...
  return {
    ...
    capability,
    requested_mutation: Boolean(mutation),
    mutation_permission: false,
    ...decision,
  };
}
...
function installConnectionRoutes(app, requireApiKey) {
  app.get('/capabilities/connections', requireApiKey, (req, res) => {
    res.json(buildConnectionsView());
  });

  app.get('/health/connections', requireApiKey, (req, res) => {
    res.json(buildConnectionsHealth());
  });
  ...
  app.get('/capabilities/connections/:id/:capability', requireApiKey, (req, res) => {
    const result = resolveCapability({
      ...
      mutation: req.query.mutation === 'true',
    });
