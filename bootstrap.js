...
function buildRuntimeViews() {
  ...
  const cycle = buildReadonlyCycleState({
    ...
    report: {
      conversion_integrity: r.conversion_integrity,
      anomalies: r.anomalies,
      daily_manager: r.daily_manager,
      mode: "shadow",
    },
  ...
  refreshPromise = collectFullLiveShadowInput({ days: Number(process.env.SHADOW_REPORT_DAYS || 30) })
    ...
    .catch((error) => {
      ...
      console.error(JSON.stringify({
        ...
        success: false,
        error: errorMessage,
        stale_result_preserved: Boolean(hadPreviousResult && state.result),
        writes_allowed: false,
      }));
      throw error;
    })
...
function wrappedExpress(...args) {
  ...
  app.get("/health/agent-shadow-summary", (req, res) => {
    if (state.status === "starting" && !state.result) {
      return safePublicJson(res.status(202), {
        success: true,
        status: "running",
        mode: "shadow",
        writes_allowed: false,
    ...
    if (state.status === "failed" && !state.result) {
      return safePublicJson(res.status(500), {
        success: false,
        status: "failed",
        mode: "shadow",
        writes_allowed: false,
        error: "shadow_report_failed",
      });
    }
  ...
  app.get("/tools/agent/shadow/live", (req, res) => {
    ...
    return res.json({
      success: true,
      mode: "shadow",
      status: refreshPromise ? "refreshing" : "completed",
      writes_allowed: false,
      started_at: state.started_at,
      finished_at: state.finished_at,
