...
test('credential-like keys are rejected recursively',()=>{
 for(const key of ['access_token','refresh_token','developer_token','client_secret','api_key','authorization']){
  assert.throws(()=>assertPublicPayloadSafe({nested:{[key]:'secret'}}),/public payload safety contract violated/);
 }
});

test('advertising object id keys are rejected from public payloads',()=>{
 for(const key of ['campaign_id','adset_id','ad_id','creative_id','ad_account_id','customer_id']){
  assert.throws(()=>assertPublicPayloadSafe({[key]:'123'}),/public payload safety contract violated/);
 }
});
