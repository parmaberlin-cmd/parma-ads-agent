...
test('Control Tower exposes specialist ownership, auth and live shadow source health',()=>{
 ...
 const tower=buildControlTower({runtimeState:state,schedulerSnapshot:{bootstrap:{status:'healthy',managed_schedule_id:'autonomous-business-loop-google-cycle'}},env:{RAILWAY_VOLUME_MOUNT_PATH:dir}});
 assert.equal(tower.schema,'parma.control_tower.v1');assert.equal(tower.next_task.specialist,'google_ads');assert.equal(tower.next_task.authorization.write,'not_applicable');assert.equal(tower.next_task.authorization.spend,'not_applicable');assert.equal(tower.source_health.shadow.meta,true);assert.ok(tower.action_registry.some(x=>x.kind==='instagram.publish'&&x.implemented===true&&x.authorization==='package_specific_instagram_authorization_required'));assert.equal(tower.recurring_bootstrap.status,'healthy');
});
...
test('Control Tower exposes recurring bootstrap fallback as missing when no snapshot is provided',()=>{
 const tower=buildControlTower({runtimeState:baseState(),env:{}});
