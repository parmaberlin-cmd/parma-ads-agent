...
test('Control Tower exposes specialist ownership, auth and live shadow source health',()=>{
 ...
 assert.equal(tower.schema,'parma.control_tower.v1');assert.equal(tower.next_task.specialist,'google_ads');assert.equal(tower.next_task.authorization.write,'not_applicable');assert.equal(tower.next_task.authorization.spend,'not_applicable');assert.equal(tower.source_health.shadow.meta,true);assert.ok(tower.action_registry.some(x=>x.kind==='instagram.publish'&&x.implemented===false));assert.equal(tower.recurring_bootstrap.status,'healthy');
});
