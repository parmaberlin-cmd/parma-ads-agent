...
test("current compatibility builder explicitly disables Advantage Audience", () => {
  const draft = buildPausedReservationDraft(validInput());
  assert.deepEqual(draft.adSet.targeting.targeting_automation, { advantage_audience: 0 });
  assert.equal(draft.campaign.status, "PAUSED");
  assert.equal(draft.adSet.status, "PAUSED");
  assert.equal(draft.ad.status, "PAUSED");
});

test("Meta compatibility defaults to current pinned generation", () => {
  assert.equal(META_API_VERSION, "v26.0");
  assert.equal(runtimeConfig({}).apiVersion, "v26.0");
  assert.equal(normalizeApiVersion("garbage"), "v26.0");
  assert.equal(normalizeApiVersion("v26.0"), "v26.0");
});

test("compatibility summary remains non-activating", () => {
