...
const {AutonomousRuntime,readState,acquireFileLock,releaseFileLock,withLockedState}=require('../autonomous-runtime');
...
test('atomic lock rejects concurrent state mutation',()=>{
 const file=tempFile(); fs.mkdirSync(path.dirname(file),{recursive:true}); const lock=acquireFileLock(file,{leaseMs:60000}); assert.equal(lock.ok,true); const second=withLockedState(file,()=>{throw new Error('must not run')},{leaseMs:60000}); assert.equal(second.locked,false); releaseFileLock(lock.lock);
});
...
test('delegation policy routes RED/unknown action to NEEDS_HUMAN',async()=>{
