...
test("conversion gate blocks optimization when Ads and GA4 materially disagree",()=>{
  const r=conversionIntegrity({googleConversions:10,ga4GoogleCpcBookings:4});
...
test("conversion gate accepts values within tolerance",()=>{
  const r=conversionIntegrity({googleConversions:10,ga4GoogleCpcBookings:9});
...
test("shadow engine recommends investigation but performs zero writes",()=>{
  const r=buildShadowDecisions({conversions:{googleConversions:8,ga4GoogleCpcBookings:2},google:{cost:20,bookings:0,baselineBookings:5},meta:{cost:12,bookings:0,baselineBookings:3}});
