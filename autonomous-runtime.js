...
function readState(file=statePath(), now=()=>Date.now()) {
  if(!fs.existsSync(file)) return emptyState(now);
  let parsed; try { parsed=JSON.parse(fs.readFileSync(file,'utf8')); } catch { const e=new Error('autonomous_state_parse_failed'); e.code='STATE_PARSE_FAILED'; throw e; }
  if(!parsed || parsed.version!==1 || !Array.isArray(parsed.objectives) || !Array.isArray(parsed.audit)){ const e=new Error('autonomous_state_invalid'); e.code='STATE_INVALID'; throw e; }
  return parsed;
}
function writeState(state,file=statePath(),now=()=>Date.now()) { state.updated_at=nowIso(now); fs.mkdirSync(path.dirname(file),{recursive:true}); const tmp=`${file}.${process.pid}.${crypto.randomUUID()}.tmp`; fs.writeFileSync(tmp,`${JSON.stringify(state)}\n`,{mode:0o600}); fs.renameSync(tmp,file); return state; }
...
function acquireFileLock(file,{now=()=>Date.now(),leaseMs=30000}={}){
  fs.mkdirSync(path.dirname(file),{recursive:true}); const lock=`${file}.lock`;
  try { fs.mkdirSync(lock); fs.writeFileSync(path.join(lock,'lease.json'),JSON.stringify({pid:process.pid,expires_at:now()+leaseMs}),{mode:0o600}); return {ok:true,lock}; } catch {
...
function recoverExpiredLeases(state,now=()=>Date.now()){
  let recovered=0; for(const o of state.objectives){ if(!ACTIVE.has(o.status))continue; for(const t of o.tasks){ if(t.status==='RUNNING' && (!t.lease?.expires_at || Date.parse(t.lease.expires_at)<=now())){ t.status='PENDING';t.lease=null;t.next_action='retry_after_crash';t.updated_at=nowIso(now);o.status='READY';o.current_task_id=null;o.next_action='recover_crashed_task';audit(state,'task_lease_recovered',{objective_id:o.id,task_id:t.id},now);recovered++; } } } return recovered;
}
function publicSnapshot(state,now=()=>Date.now()){
  const active=state.objectives.find(o=>ACTIVE.has(o.status))||null; const current=active?.tasks.find(t=>t.id===active.current_task_id)||null; const next=active?selectTask(active,now):null;
...
class AutonomousRuntime {
  ...
  submit(input){ const r=withLockedState(this.file,(s)=>{ const o=normalizeObjective(input,this.now); s.objectives.push(o); audit(s,'objective_submitted',{objective_id:o.id,task_count:o.tasks.length},this.now); return o; },{now:this.now,leaseMs:this.leaseMs}); if(!r.locked){const e=new Error('state_lock_busy');e.code='STATE_LOCK_BUSY';throw e;} return r.value; }
  ...
  snapshot(){ const s=readState(this.file,this.now); const p=publicSnapshot(s,this.now); p.storage=storageStatus(this.env); return p; }
  start(){ if(this.timer) return; withLockedState(this.file,s=>{recoverExpiredLeases(s,this.now);s.runner.status='RUNNING';s.runner.started_at=s.runner.started_at||nowIso(this.now);s.runner.heartbeat_at=nowIso(this.now);audit(s,'runner_started',{worker_id:this.workerId},this.now);},{now:this.now,leaseMs:this.leaseMs}); this.tick().catch(()=>{}); this.timer=setInterval(()=>this.tick().catch(()=>{}),this.tickMs); if(this.timer.unref)this.timer.unref(); }
  stop(){ if(this.timer)clearInterval(this.timer);this.timer=null; }
  scheduleRepair(s,o,t,reason){ const wait=backoffMs(t.attempts);t.status='RETRY_WAIT';t.retry={backoff_ms:wait,next_attempt_at:new Date(this.now()+wait).toISOString()};t.next_action='auto_correct_then_revalidate';o.status='RETRY_WAIT';o.current_task_id=null;o.next_action='retry_after_backoff';audit(s,'task_repair_scheduled',{objective_id:o.id,task_id:t.id,attempt:t.attempts,reason,backoff_ms:wait},this.now); }
  async tick(){
    const claim=withLockedState(this.file,s=>{
      recoverExpiredLeases(s,this.now); s.runner.heartbeat_at=nowIso(this.now); if(s.runner.kill_switch){s.runner.status='STOPPED'; return null;} s.runner.status='RUNNING';
      const obj=s.objectives.find(o=>ACTIVE.has(o.status)); if(!obj) return null;
      let task=selectTask(obj,this.now); if(!task){ if(obj.tasks.every(t=>t.status==='DONE')){obj.status='DONE';obj.stop_reason='objective_verified';obj.completed_at=nowIso(this.now);obj.updated_at=nowIso(this.now);obj.current_task_id=null;obj.next_action=null;audit(s,'objective_done',{objective_id:obj.id},this.now);} return null; }
      const auth=authorizeAutonomy({name:task.kind},{kill_switch:s.runner.kill_switch,human_approved:false});
      audit(s,'delegation_decision',{objective_id:obj.id,task_id:task.id,allowed:auth.allowed,reason:auth.reason,action_class:auth.action_class},this.now);
    ...
    try { if(typeof handler!=='function'){ const e=new Error('capability_handler_unavailable'); e.code='CAPABILITY_UNAVAILABLE'; throw e; } result=await handler({objective_id:c.objective_id,task:c.task}); }
      ...
      const cls=classifyApiFailure(error); const transient=cls==='transient'; return withLockedState(this.file,s=>{ const o=s.objectives.find(x=>x.id===c.objective_id);const t=o?.tasks.find(x=>x.id===c.task.id);if(!o||!t||t.status!=='RUNNING'||t.lease?.owner!==this.workerId)return; const err={at:nowIso(this.now),class:cls||'unknown',code:sanitizeText(error.code||'execution_failed',80)};t.errors.push(err);t.lease=null;t.updated_at=nowIso(this.now);s.runner.last_error={objective_id:o.id,task_id:t.id,...err}; if(transient&&t.attempts<t.max_attempts)this.scheduleRepair(s,o,t,'transient_execution_failure');else{t.status='BLOCKED_EXTERNAL';t.stop_reason=transient?'retry_limit_reached':'provider_or_capability_blocked';o.status='BLOCKED_EXTERNAL';o.stop_reason=t.stop_reason;o.next_action='external_unblock';audit(s,'blocked_external',{objective_id:o.id,task_id:t.id,reason:t.stop_reason},this.now);} },{now:this.now,leaseMs:this.leaseMs});
    }
    withLockedState(this.file,s=>{ const o=s.objectives.find(x=>x.id===c.objective_id);const t=o?.tasks.find(x=>x.id===c.task.id);if(!o||!t||t.status!=='RUNNING'||t.lease?.owner!==this.workerId)return; const valid=result?.validated===true; const correctable=result?.correctable===true; t.evidence.push(sanitizeEvidence(result?.evidence||{}));t.lease=null;t.updated_at=nowIso(this.now);audit(s,'validation_result',{objective_id:o.id,task_id:t.id,validated:valid,correctable},this.now); if(valid){t.status='DONE';t.completed_at=nowIso(this.now);t.next_action='next_task';o.current_task_id=null;o.status='READY';o.next_action='select_next_task';audit(s,'task_done',{objective_id:o.id,task_id:t.id,attempt:t.attempts},this.now);}else if(correctable&&t.attempts<t.max_attempts){this.scheduleRepair(s,o,t,'correctable_validation_failure');}else{t.status='BLOCKED_EXTERNAL';t.stop_reason='validation_failed';o.status='BLOCKED_EXTERNAL';o.stop_reason='validation_failed';o.next_action='external_unblock';audit(s,'validation_failed',{objective_id:o.id,task_id:t.id},this.now);} },{now:this.now,leaseMs:this.leaseMs});
  }
}

module.exports={AutonomousRuntime,TERMINAL,ACTIVE,statePath,storageStatus,emptyState,normalizeObjective,readState,writeState,withLockedState,acquireFileLock,releaseFileLock,dependenciesMet,selectTask,backoffMs,recoverExpiredLeases,publicSnapshot,sanitizeEvidence};
