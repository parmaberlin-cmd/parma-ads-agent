...
const { decideNextMetaStep } = require("./meta-preflight-decision");
...
function normalizeAccountId(value) {
  ...
  if (/^act_\d{1,30}$/.test(text)) return text;
  if (/^\d{1,30}$/.test(text)) return `act_${text}`;
  return null;
}

function parseFutureStart(value, now = Date.now()) {
  const date = new Date(String(value || ""));
  if (Number.isNaN(date.getTime()) || date.getTime() <= now + 15 * 60 * 1000) return null;
...
function cleanError(error) {
  ...
  return {
    message: String(graph.message || error?.message || "meta_preflight_failed")
...
function runtimeConfig(env = process.env) {
  const businessTimezone = env.META_BUSINESS_TIMEZONE || env.META_AD_TIMEZONE || DEFAULT_BUSINESS_TIMEZONE;
  return {
    accessToken: env.META_ACCESS_TOKEN || null,
    adAccountId: normalizeAccountId(env.META_AD_ACCOUNT_ID),
    apiVersion: normalizeApiVersion(env.META_API_VERSION),
    dsaBeneficiary: env.META_AD_DSA_BENEFICIARY || null,
    dsaPayor: env.META_AD_DSA_PAYOR || null,
    writeGateEnabled: env.META_PAUSED_DRAFT_WRITES_ENABLED === "true",
    businessTimezone,
    expectedTimezone: businessTimezone,
    expectedCurrency: env.META_AD_CURRENCY || "EUR",
  };
}

function createReadTransport({ accessToken, apiVersion = META_API_VERSION, client = axios }) {
  const http = client.create
    ? client.create({ baseURL: `https://graph.facebook.com/${normalizeApiVersion(apiVersion)}`, timeout: 20000 })
    : client;
  return {
...
async function inspectAccountContext(transport, config) {
  const account = await transport.get(`/${config.adAccountId}`, {
    fields: "account_status,currency,timezone_name",
  });
  const timezone = String(account?.timezone_name || "");
  const currency = String(account?.currency || "").toUpperCase();
  const businessTimezone = config.businessTimezone || config.expectedTimezone || DEFAULT_BUSINESS_TIMEZONE;
  const blockers = [];

  if (!timezone) blockers.push("account_timezone_unknown");
  if (!currency) blockers.push("account_currency_unknown");
  else if (currency !== config.expectedCurrency) blockers.push("account_currency_mismatch");

  return {
    readable: true,
    timezone_name: timezone,
    expected_timezone: businessTimezone,
    business_timezone: businessTimezone,
    timezone_match: timezone === businessTimezone,
    schedule_conversion_required: Boolean(timezone && timezone !== businessTimezone),
    schedule_conversion_safe: timezone === businessTimezone,
    currency,
    expected_currency: config.expectedCurrency,
    currency_match: currency === config.expectedCurrency,
    account_status_present: account?.account_status != null,
    blockers,
  };
}
...
function finalizeResult(result) {
  return {
    ...result,
    decision: decideNextMetaStep(result),
...
function validateScheduleForAccount({ start, durationDays, account, businessTimezone }) {
  if (!account.timezone_name) {
    return { safe: false, reason: "account_timezone_unknown" };
  }
  if (account.timezone_name === businessTimezone) {
    return { safe: true, converted: false, reason: null };
  }
  return {
    ...convertDailyWindow({
      startsAt: start,
      durationDays,
      sourceTimeZone: businessTimezone,
      targetTimeZone: account.timezone_name,
      startHour: 17,
      endHour: 23,
    }),
    converted: true,
...
async function executeRuntimeMetaPreflight({ env = process.env, startsAt, httpClient = axios } = {}) {
  ...
  account.schedule_conversion_safe = Boolean(schedule.safe);
  account.schedule_conversion_required = Boolean(schedule.converted);
  if (!schedule.safe && schedule.reason && !account.blockers.includes(schedule.reason)) {
    account.blockers.push(schedule.reason);
  }
  ...
  if (!schedule.safe) {
    return finalizeResult({
      ...
      blockers: [...new Set(account.blockers)],
    });
  }
  ...
  return finalizeResult({
    ...preflight,
    ready: preflight.ready && account.blockers.length === 0,
    account,
    api_version: config.apiVersion,
...
function registerMetaRealPreflightRoute(app, { authorized, env = process.env, httpClient = axios } = {}) {
  app.get("/tools/meta/reservation-draft/real-preflight", async (req, res) => {
    ...
    try {
      const result = await executeRuntimeMetaPreflight({ env, startsAt: req.query?.starts_at, httpClient });
      return res.status(result.ready ? 200 : 409).json({
        ...result,
        endpoint: "meta_real_preflight",
        write_operation_performed: false,
...
module.exports = {
  ...
  registerMetaRealPreflightRoute,
};
