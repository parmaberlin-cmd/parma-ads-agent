...
function localHandlers(env=process.env){
  return {
    ...
    generate_report: async ({objective_id,task}) => {
      const state=readState(statePath(env));const objective=state.objectives.find(x=>x.id===objective_id);const controlled=firstDone(objective,'google_ads.execute_authorized')?.evidence?.[0]||null;
