...
function detectAnomalies({ current = {}, baseline = {}, access = {} } = {}) {
  const anomalies = [];
  const push = (code, severity, reason, channel = "cross_channel") => anomalies.push({ code, severity, reason, channel });
  ...
  const currentSpend = numberOrZero(current.spend), baselineSpend = numberOrZero(baseline.spend), currentClicks = numberOrZero(current.clicks), currentConversions = numberOrZero(current.conversions), baselineConversions = numberOrZero(baseline.conversions), currentCpc = numberOrZero(current.cpc), baselineCpc = numberOrZero(baseline.cpc);
...
function createDecisionJournalEntry({ timestamp = new Date().toISOString(), channel, evidenceWindow, evidence = {}, dataQuality, diagnosis, confidence, expectedEffect, proposedAction, requiresAuthorization = false, approvalState = "not_requested", execution = null, verification = null, measurementWindow = null, outcome = null } = {}) {
  if (!channel || !diagnosis || !proposedAction) throw new Error("channel, diagnosis and proposedAction are required");
  return { timestamp, channel, evidence_window: evidenceWindow || null, evidence, data_quality: dataQuality || "unknown", diagnosis, confidence: confidence || "unknown", expected_effect: expectedEffect || null, proposed_action: proposedAction, requires_authorization: Boolean(requiresAuthorization), approval_state: approvalState, execution, verification, measurement_window: measurementWindow, outcome };
}
...
function finalizeDecisionJournalEntry(entry, { execution, verification, outcome } = {}) {
  ...
  return { ...entry, execution: execution ?? entry.execution, verification: verification ?? entry.verification, outcome: outcome ?? entry.outcome };
}

module.exports = { assessConversionIntegrity, createDecisionJournalEntry, detectAnomalies, finalizeDecisionJournalEntry };
