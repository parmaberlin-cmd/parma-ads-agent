...
async function verifyPublishedMedia({transport,mediaId,expectedMediaType=null,expectedUsername=null}={}){requireTransport(transport);const r=await transport.get(`/${numericId(mediaId,'media id')}`,{fields:'id,media_type,media_product_type,permalink,timestamp,username'});const productType=String(r?.media_product_type||r?.media_type||'').toUpperCase();const expectedType=expectedMediaType?String(expectedMediaType).toUpperCase():null;const username=String(r?.username||'').toLowerCase();const usernameMatches=!expectedUsername||username===String(expectedUsername).toLowerCase();const typeMatches=!expectedType||productType===expectedType||(expectedType==='REELS'&&productType==='VIDEO');const requiresPermalink=expectedType!=='STORIES';return {published:Boolean(r?.id&&usernameMatches&&typeMatches&&(!requiresPermalink||r?.permalink)),verification:{id_present:Boolean(r?.id),username_matches:usernameMatches,media_type_matches:typeMatches,permalink_required:requiresPermalink,permalink_present:Boolean(r?.permalink)},media:{id:r?.id||null,media_type:r?.media_type||null,media_product_type:r?.media_product_type||null,permalink:r?.permalink||null,timestamp:r?.timestamp||null,username:r?.username||null}};}
